...
export default {
  ...
  mounted: function () {
    axios.get(this.global.server + '/api/tools/queryMock').then(json => {
      if (json.data['cstest_data']['status'] === 0) {
        this.options = json.data['cstest_data']['data']
      }
    })
  },
  computed: {
  ...
  watch: {
    ukey: function () {
      ...
      if (flag === 0) {
        this.form.head = '{"Access-Control-Allow-Origin": "https://test.djtest.cn","Access-Control-Allow-Credentials": "true"}'
  ...
  methods: {
    formInit () {
      var formInit = {
        key: '',
        desc: '',
        head: '{"Access-Control-Allow-Origin": "https://test.djtest.cn","Access-Control-Allow-Credentials": "true"}',
        ...
        if (valid) {
          ...
          axios({
            ...
            .then(response => {
              ...
              if (response.data.cstest_status === true) {
                this.resultFlag = true
                this.result = response.data.cstest_data.data
                this.$message({
                  showClose: true,
                  message: '提交成功！',
                  type: 'success'
                ...
                self.errorflag = true
                self.errorinfo = response.data.cstest_message
              }
            })
            .catch((error) => {
              console.log(error)
